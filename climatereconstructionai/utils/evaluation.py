...
def create_outputs(data_dict, dataset, eval_path, output_names, stat_target, i_model, split, i_start, i_end):
    ...
    m_label = "." + str(i_model).zfill(4)
    suffix = m_label + "-" + str(split + 1).zfill(4)
    ...
    for j in range(len(eval_path)):
        ...
        for cname in cnames:
            ...
            ds[data_type] = xr.DataArray(data_dict[cname].to(torch.device('cpu')).detach().numpy()[:, j, :, :], dims=dataset.xr_dss[ind][2])
            ds["time"] = dataset.xr_dss[ind][0]["time"][i_start: i_end]

            ds = reformat_dataset(dataset.xr_dss[j][0], ds, data_type)
        ...
        for time_step in cfg.plot_results:
            if time_step >= i_start and time_step < i_end:
                ...
                plot_data(dataset.xr_dss[j][1].coords, [data_dict[p][time_step - i_start, j, :, :].squeeze() for p in pnames],
                      ["Original", "Reconstructed"], output_name, data_type, str(dataset.xr_dss[j][0]["time"][time_step].values), *cfg.dataset_format["scale"])
